...
import { ALLOWED_AVATAR_TYPES, MAX_AVATAR_SIZE } from '../validators/user.validator'
...
export async function uploadAvatar(userId: string, file: File) {
  ...
  if (existing?.avatar) {
    await deleteFile(BUCKETS.AVATAR, extractPath(existing.avatar, BUCKETS.AVATAR)).catch(() => null)
  }
...
export async function submitVerification(userId: string, nik: string, ktpFile: File) {
  if (!ALLOWED_KTP_TYPES.includes(ktpFile.type as (typeof ALLOWED_KTP_TYPES)[number])) {
    throw new AppError(400, 'Format file KTP tidak valid. Gunakan JPEG atau PNG')
  }
  ...
  await prisma.user.update({
    where: { id: userId },
    data: { nik, ktpUrl: path, verificationStatus: 'PENDING', verificationRejectReason: null },
  })
